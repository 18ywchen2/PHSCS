...
from preproccessing import dataset_Peng_Qian_2
...
def read_examples(args=None):
    ...
    if args.SWCsType in ["TP", "BN", "DE", "EF", "UC", "RE", "OF", "SE"]:
        codes, Labels = dataset_Peng_Qian_2('Dataset',
...
def convert_examples_to_features(examples, tokenizer, args):
    ...
    for idx, example in tqdm(enumerate(examples), total=len(examples)):
        ...
        source_tokens = [y for x in source_tokens for y in x][:args.max_source_length - 2]
...
def main(head, SWCType):
    ...
    train_ids = torch.tensor(sample[:, 0:args.max_source_length])
    train_mask = torch.tensor(sample[:, args.max_source_length:args.max_source_length * 2])
    train_pos = torch.tensor(sample[:, args.max_source_length * 2:args.max_source_length * 3])
    ...
    test_ids = torch.tensor(sample[:, 0:args.max_source_length])
    test_mask = torch.tensor(sample[:, args.max_source_length:args.max_source_length * 2])
    test_pos = torch.tensor(sample[:, args.max_source_length * 2:args.max_source_length * 3])
...
if __name__ == "__main__":
    ...
    for head in heads:
        for SWCType in SWCsType:
            result = main(head=head, SWCType=SWCType)
